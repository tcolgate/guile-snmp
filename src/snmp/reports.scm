...
(define-method  (value (var <snmp-reports-result>))
  (slot-ref var 'value))
(define-method  (value-bytes (var <snmp-reports-result>))
  (slot-ref (rawvarbind var) 'value-bytes))
      ...
      (lambda( . msg)
        (if (not (null? (results result-set)))
          (cond
              ...
              (let ((var (assoc (car msg) (results result-set))))
                (if (not var)
                  ...
                  (cond
                    ...
                    ((equal? (cdr msg) (list 'value-bytes))   (value-bytes(cdr var)))
            ...
            ((equal? (car msg) 'value-bytes)   (value-bytes (cdr (car (results result-set)))))
...
(define-method  (value-bytes (varset <snmp-reports-result-set>))
  (varset  'value-bytes))
(define-method  (value-bytes (varset <snmp-reports-result-set>) (item <oid>))
  (varset item 'value-bytes))
